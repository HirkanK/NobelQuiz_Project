...
$subject = $timerQuery = $con->prepare("SELECT duration_minutes FROM quiz_status WHERE subject = ?");
$timerQuery->bind_param("s", $subject);
$timerQuery->execute();
$timerResult = $timerQuery->get_result();
$durationMinutes = $timerResult->fetch_assoc()['duration_minutes'] ?? 60;
...
// Check if this subject's quiz is live
$checkLive = $con->prepare("SELECT is_active FROM quiz_status WHERE subject = ?");
$checkLive->bind_param("s", $subject);
$checkLive->execute();
$liveResult = $checkLive->get_result();

if (!$liveResult->fetch_assoc()['is_active']) {
    echo "<h3>⛔ Quiz for '$subject' is not live yet. Please wait for Admin to push it.</h3>";
    exit();
}
...
<head>
  ...
  <script>
let remainingTime = <?= $durationMinutes ?> * 60; // in seconds
...
function startTimer() {
  ...
  const interval = setInterval(() => {
    ...
    timerDisplay.textContent = `Time Left: ${mins}m ${secs}s`;
...
<body>
<h3 id="timer" style="color:red; font-weight:bold;">Time Left: 60m 0s</h3>
